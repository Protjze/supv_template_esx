			...
			if element.value == "return" then
				...
				ESX.OpenContext("right", elements2, function(menu2,element2)
					...
					elseif element2.value == "fire" then
						...
						ESX.TriggerServerCallback(('esx_society:%s'):format(eventName[selected].set), function()
							OpenEmployeeList(society, options, selected)
						end, employee.identifier, selected == 'job' and 'unemployed' or Config.DoubleJob.default.list.name , 0, 'fire')
...
function OpenPromoteMenu(society, employee, options, selected)
	ESX.TriggerServerCallback(('esx_society:%s'):format(eventName[selected].get), function(job)
		if not job then
			return
		end
		...
		for i=1, #job.grades, 1 do
			...
			elements[#elements+1] = {icon = "fas fa-user", title = gradeLabel, value = job.grades[i].grade, selected = (employee[selected].grade == job.grades[i].grade)}
		end
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if element.value == "confirm" then
				...
				elseif amount > Config.MaxSalary then
					...
					ESX.CloseContext()
					ESX.TriggerServerCallback(('esx_society:%s'):format(eventName[selected].setSalary), function()
						OpenManageSalaryMenu(society, options, selected)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if element.value == "confirm" then
				if menu.eles[2].inputValue then
					...
					ESX.TriggerServerCallback(('esx_society:%s'):format(eventName[selected].setLabel), function()
					OpenManageGradesMenu(society, options, selected)
