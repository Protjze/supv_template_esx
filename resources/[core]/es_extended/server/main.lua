...
function createESXPlayer(identifier, playerId, data)
  ...
  if not Config.Multichar then
    MySQL.prepare(newPlayer, {json.encode(accounts), identifier, defaultGroup}, function()
      ...
      if not ReallyNewPlayer[playerId] then ReallyNewPlayer[playerId] = playerId end
    end)
  else
...
function ESX.SaveAll()
  Core.SavePlayers()
end

if Config.OxInventory then
  if next(Config.StartingItems) then
    RegisterNetEvent('esx:addStartingItems', function()
      local _source = source
      local xPlayer = ESX.GetPlayerFromId(_source)
      if xPlayer then
        if ReallyNewPlayer[_source] then
          for k,v in pairs(Config.StartingItems) do
            exports.ox_inventory:AddItem(_source, k, v)
          end
