...
AddEventHandler('esx_society:checkSocietyBalance', function(society)
	...
	if (xPlayer.job.name ~= society.name) or (Config.DoubleJob and xPlayer[Config.DoubleJob.name].name ~= society.name) then
...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	...
	elseif Config.DoubleJob and Factions[society] then
		local xPlayers = ESX.GetExtendedPlayers(Config.DoubleJob.name, society)
		for i=1, #(xPlayers) do 
			local xPlayer = xPlayers[i]
			...
			employees[#employees+1] = {
				name = name,
				identifier = xPlayer.identifier,
				[Config.DoubleJob.name] = {
					name = society,
					label = xPlayer[Config.DoubleJob.name].label,
					grade = xPlayer[Config.DoubleJob.name].grade,
					grade_name = xPlayer[Config.DoubleJob.name].grade_name,
					grade_label = xPlayer[Config.DoubleJob.name].grade_label
				}
			}
		end
			
		local query = ("SELECT identifier, %s FROM `users` WHERE `%s`= ? ORDER BY %s DESC"):format(Config.DoubleJob.database.users_dj_grade, Config.DoubleJob.database.users_dj_name, Config.DoubleJob.database.users_dj_grade)
				...
				if not alreadyInTable then
					...
					employees[#employees+1] = {
						...
						[Config.DoubleJob.name] = {
							...
							grade = row.job_grade,
							grade_name = Factions[society].grades[tostring(row.job_grade)].name,
							grade_label = Factions[society].grades[tostring(row.job_grade)].label
						}
					}
				end
			end
...
function isPlayerBoss(playerId, society)
	...
	if (xPlayer.job.name == society and xPlayer.job.grade_name == 'boss') or (Config.DoubleJob and xPlayer[Config.DoubleJob.name].name == society and xPlayer[Config.DoubleJob.name].grade_name == 'boss') then
		return true
	else
